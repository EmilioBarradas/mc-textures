...
for (const tag of tags) {
    ...
    const tagNormalized = tag.split(':')[1].toUpperCase();

    if (ignored.includes(tagNormalized)) continue;

    let pos = config.ids[tagNormalized]?.pos;

    if (typeof pos === 'undefined') {
        pos = config.ids[deviations[tagNormalized]]?.pos;
    }

    if (typeof pos === 'undefined') continue;

    positions[tagNormalized] = {
